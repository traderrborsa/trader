...
function UserMenu() {
  ...
  const copyCustomerId = useCallback(async () => {
    if (!user?.customerSlug) return;
    try {
      await navigator.clipboard.writeText(user.customerSlug);
      setCopiedId(true);
          ...
          <div className="border-b border-border px-4 py-3">
            ...
            <p className="mt-1 text-xs text-muted">
              ...
              <MoneyAmount
                ...
                opts={{ fractionDigits: 0, currency: portfolio.currency }}
                size="inline"
                as="span"
        ...
        <div className="ml-auto flex shrink-0 items-center gap-2">
          ...
          {user ? (
            <>
              ...
              <Link
                href="/portfolio"
                className="hidden h-[36px] max-w-[min(100%,14rem)] min-w-0 items-center gap-1.5 rounded-full border border-border-strong bg-card px-4 py-2 text-sm leading-none transition hover:border-border-strong sm:inline-flex"
              >
                <span className="shrink-0 text-subtle">Bakiye:</span>
                <MoneyAmount
                  amount={portfolio.balance}
                  opts={{ fractionDigits: 0, currency: portfolio.currency }}
                  size="header"
