...
function SummaryRow({
  ...
  return (
    <div className="flex items-center justify-between gap-4 py-3">
      ...
      <span
        className={`min-w-0 max-w-[60%] overflow-hidden text-right tabular-nums break-all [overflow-wrap:anywhere] ${emphasize ? 'text-base font-bold' : 'text-sm font-semibold'} ${valueClassName ?? 'text-foreground'}`}
        title={value}
      >
        {value}
      </span>
  ...
  return (
    <section className="corp-card mb-6 overflow-hidden p-0">
      <div className="border-b border-border/60 bg-accent/5 px-5 py-6 sm:px-6">
        ...
        <MoneyAmount amount={balances.totalBalance} size="hero" className="mt-2" />
