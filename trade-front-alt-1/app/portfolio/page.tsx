    ...
    <div className={`flex min-h-screen flex-col bg-background text-foreground ${MOBILE_NAV_PB}`}>
      ...
      <main className="mx-auto w-full max-w-5xl flex-1 px-4 py-5 sm:px-6">
        {portfolioLoading ? (
          ...
          <>
            <div className="mb-5 min-w-0 overflow-hidden rounded-2xl bg-card p-5 shadow-sm sm:p-6">
              <p className="text-sm text-muted">Toplam bakiye</p>
              <MoneyAmount amount={balances.totalBalance} size="hero" className="mt-1" />
              <p className="mt-2 min-w-0 break-all text-sm text-muted [overflow-wrap:anywhere]">
                Bonus {formatMoney(balances.bonusIncome)} · Kredi{' '}
                {formatMoney(balances.creditIncome)} · Nakit{' '}
                {formatMoney(balances.cashBalance)}
              </p>
              <p className="mt-3 min-w-0 break-all text-xs text-muted [overflow-wrap:anywhere]">
            ...
            <div className="mb-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
              ...
              <div className={`${CARD} min-w-0 p-4`}>
                ...
                <MoneyAmount amount={balances.cashBalance} size="compact" className="mt-1" />
              </div>
              <div className={`${CARD} min-w-0 p-4`}>
                <p className="text-xs font-medium text-muted">Açık K/Z</p>
                {portfolio.positions.length === 0 ? (
                  <p className="mt-1 text-lg font-bold tabular-nums text-muted">—</p>
                ) : (
                  <MoneyAmount
                    amount={unrealizedTotal}
                    size="compact"
                    className={`mt-1 ${
                      unrealizedTotal >= 0 ? 'text-positive' : 'text-negative'
                    }`}
